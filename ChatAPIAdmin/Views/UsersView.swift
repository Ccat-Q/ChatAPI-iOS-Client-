...
private struct UserDetailView: View {
    ...
    var body: some View { Form { Section("Account") { LabeledContent("Username", value: user.username); LabeledContent("Role", value: user.role); LabeledContent("Status", value: user.isActive ? "Active" : "Disabled") }; Section { Button(user.isActive ? "Disable User" : "Enable User", role: user.isActive ? .destructive : nil) { confirmDisable = true } } }.navigationTitle(user.username).confirmationDialog("Change user access?", isPresented: $confirmDisable) { Button(user.isActive ? "Disable" : "Enable", role: user.isActive ? .destructive : nil) {} } message: { Text("This immediately changes the user's ability to access the instance.") } }
}
